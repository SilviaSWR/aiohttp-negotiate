...
class NegotiateMixin(object):
    ...
    def get_context(self, host):
        service_name = gssapi.Name(
            self.negotiate_service_name
            or "{0}@{1}".format(self.negotiate_service, host),
            gssapi.NameType.hostbased_service,
        )
        logger.debug("Service name: {0}".format(service_name))
        if self.negotiate_client_name:
            creds = gssapi.Credentials(
                name=gssapi.Name(self.negotiate_client_name), usage="initiate"
            )
        else:
            creds = None
        logger.debug("Credentials: {0}".format(creds))
        return gssapi.SecurityContext(name=service_name, creds=creds)
